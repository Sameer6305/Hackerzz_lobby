...
const JOBS = [
  ...
  },
  {
    ...
    logo: '🟣', color: 'from-primary-600 to-primary-700',
  },
];
      ...
      <div className="max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            <div className="flex gap-1 surface-card rounded-xl p-1 border border-theme">
              <button
                onClick={() => setActiveTab('internships')}
                className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-all
                  ${activeTab === 'internships' ? 'bg-indigo-600/20 text-indigo-400' : 'text-label hover:text-heading'}`}
              >
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-all
                  ${activeTab === 'jobs' ? 'bg-indigo-600/20 text-indigo-400' : 'text-label hover:text-heading'}`}
              >
            ...
            <div className="relative flex-1">
              ...
              <input
                className="w-full pl-10 pr-4 py-2.5 rounded-xl surface-card border border-theme text-heading text-sm placeholder:text-hint focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
          ...
          <div className="space-y-3">
            {filtered.map((item, i) => (
              <motion.div
                ...
                className="surface-card border border-theme rounded-xl p-5 hover:border-theme-secondary transition-all group"
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h3 className="text-base font-semibold text-heading group-hover:text-indigo-400 transition">
                          {item.role}
                        </h3>
                        <p className="text-sm text-label mt-0.5">{item.company}</p>
                      </div>
                    ...
                    <div className="flex flex-wrap items-center gap-4 mt-3 text-xs text-hint">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-3">
                      {item.skills.map((skill) => (
                        <span key={skill} className="px-2 py-0.5 rounded-md surface-elevated text-label text-[11px]">
                          {skill}
                        </span>
                      ))}
                    </div>
            ...
            {filtered.length === 0 && (
              <div className="surface-card border border-theme rounded-xl p-12 text-center">
                <Search size={40} className="text-hint mx-auto mb-3" />
                <p className="text-label text-sm">No results found for "{search}"</p>
              </div>
            )}
          </div>
        </motion.div>
